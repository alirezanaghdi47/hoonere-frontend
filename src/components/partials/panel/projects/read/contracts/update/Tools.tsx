...
export const Article = ({children, article, updateProjectContractForm}) => {
    const {part, currentPart, changePart, resetPart, changeCurrentPart} = usePart(null, "read");
    ...
    useEffect(() => {
        if (article && Object.keys(article).length > 0) {
            changePart({
                ...
                sectionsLength: updateProjectContractForm.values.sections.filter(item => item.article_number === article.number).length + addonSectionsLength,
            });
        }
    }, [updateProjectContractForm.values.sections]);
...
export const Section = ({children, article, section, updateProjectContractForm}) => {
    const {part, currentPart, changePart, resetPart, changeCurrentPart} = usePart(null, "read");
    ...
    useEffect(() => {
        if (article && Object.keys(article).length > 0 && section && Object.keys(section).length > 0) {
            changePart({
                ...
                notesLength: updateProjectContractForm.values.notes.length
            });
        }
    }, [updateProjectContractForm.values.notes]);
...
export const CreateArticle = ({part, resetPart, updateProjectContractForm}) => {
    const createArticleForm = useFormik({
        ...
        onSubmit: async (result, {resetForm}) => {
            const newArray = updateProjectContractForm.values.articles.toSpliced(part?.articlesLength - 1, 0, {
            ...
            updateProjectContractForm.setFieldValue("articles", newArray);
...
export const CreateSection = ({part, resetPart, updateProjectContractForm}) => {
    const createSectionForm = useFormik({
        ...
        onSubmit: async (result, {resetForm}) => {
            const newArray = [...updateProjectContractForm.values.sections, {
            ...
            updateProjectContractForm.setFieldValue("sections", newArray);
...
export const CreateNote = ({part, resetPart, updateProjectContractForm}) => {
    const createNoteForm = useFormik({
        ...
        onSubmit: async (result, {resetForm}) => {
            const newArray = [...updateProjectContractForm.values.notes, {
                article_number: part?.articleNumber,
            ...
            updateProjectContractForm.setFieldValue("notes", newArray);
