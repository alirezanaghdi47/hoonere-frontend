...
const Jobs = ({allJobAction , updateOccupationForm, changeCurrentPart}) => {
    return (
                ...
                <div className="row gy-5">
                    {
                        updateOccupationForm?.values?.fields_of_activity.map((foa, i) =>
                            <JobCard
                                key={i}
                                group={allJobAction?.data?.data?.fieldsOfActivity.find(item => parseInt(item.id) === parseInt(foa.foa_parent_id))}
                                title={allJobAction?.data?.data?.fieldsOfActivity.find(item => parseInt(item.id) === parseInt(foa.foa_child_id))}
