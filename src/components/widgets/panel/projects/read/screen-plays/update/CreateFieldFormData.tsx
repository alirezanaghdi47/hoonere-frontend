...
const CreateFieldFormData = ({updateProjectScreenPlayForm, resetPart}) => {
    const createFieldForm = useFormik({
        ...
        onSubmit: async (result, {resetForm}) => {
            const oldFields = updateProjectScreenPlayForm.values.fields;
            const newFields = oldFields.filter(foa => JSON.stringify(foa) !== JSON.stringify(result));

            newFields.push(result);

            await updateProjectScreenPlayForm.setFieldValue("fields", newFields);
