...
const CreatePaymentModal = ({modal, _handleHideModal, createProjectContractInsertionForm}) => {
    const createPaymentModal = useFormik({
        ...
        onSubmit: async (result, {resetForm}) => {
            const totalPercent = createProjectContractInsertionForm.values.articles.find(item => item.number === modal?.data?.article.number)?.payments.reduce((acc, item) => {
            ...
            const newArray = [...createProjectContractInsertionForm.values.articles.find(item => item.number === modal?.data?.article.number)?.payments, {
                percent: Number(result.percent),
                date: result.date
            }];

            createProjectContractInsertionForm.setFieldValue(`articles[${createProjectContractInsertionForm.values.articles.findIndex(item => item.number === modal?.data?.article.number)}].payments`, newArray);
                ...
                <div
                    ...
                    <Form.Group>
                        ...
                        <DatePicker
                            id="date"
                            name="date"
                            minDate={createProjectContractInsertionForm.values.articles.find(item => item.number === modal?.data?.article.number - 2)?.start_date}
                            maxDate={createProjectContractInsertionForm.values.articles.find(item => item.number === modal?.data?.article.number - 2)?.end_date}
